...
export function GalleryGrid({ t }: { t: Content }) {
  ...
  /* Animazione d'ingresso delle tile ad ogni cambio filtro */
  useEffect(() => {
    ...
    <>
          ...
          <button
            ...
            className={`tech-label rounded-full border px-4 py-2.5 transition-colors duration-300 ${
              filter === c.key
                ...
                : "border-frost-deep text-steel hover:border-cyan-deep hover:text-cyan-deep"
            }`}
          >
            {c.label}
          </button>
        ...
        {items.map((item) => (
          <li key={item.id}>
            ...
            >
              ...
              <span
                aria-hidden
                className="absolute inset-0 bg-gradient-to-t from-deep/70 via-transparent to-transparent opacity-80"
              />
              <span className="absolute inset-x-0 bottom-0 flex flex-col gap-1 p-5">
                <span className="tech-label text-cyan">{categoryLabel(item.categoryKey)}</span>
                <span className="font-display text-lg font-medium text-white">{item.title}</span>
              </span>
            </button>
          </li>
        ))}
      ...
      {active && (
        <div
          ...
          className="fixed inset-0 z-[85] flex items-center justify-center bg-deep/90 p-5 backdrop-blur-sm"
          onClick={close}
        >
          <div
            data-lightbox-panel
          ...
          >
            ...
            />
            <div className="flex items-center justify-between bg-ice px-6 py-4">
              <div>
                <p className="tech-label text-cyan-deep">{categoryLabel(active.categoryKey)}</p>
                <p className="font-display mt-1 font-semibold text-deep">{active.title}</p>
              </div>
              <button
                ref={closeBtnRef}
                type="button"
                onClick={close}
                className="rounded-full border border-deep/15 px-4 py-2 text-sm font-medium text-deep transition-colors hover:border-cyan-deep hover:text-cyan-deep"
              >
                {t.gallery.close} ✕
              </button>
            </div>
