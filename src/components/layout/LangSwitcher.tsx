...
type Props = {
  ...
  light?: boolean;
  className?: string;
  /** Nome del gruppo, localizzato (era una stringa trilingue fissa). */
  label: string;
};

/** Switcher IT / DE / EN: sostituisce il prefisso locale del path corrente. */
export function LangSwitcher({ current, light = false, className = "", label }: Props) {
    ...
    >
      {locales.map((locale, i) => (
        <span key={locale} className="flex items-center">
          {i > 0 && (
            <span aria-hidden className={light ? "text-white/50" : "text-steel/60"}>
              /
            </span>
          )}
          <Link
